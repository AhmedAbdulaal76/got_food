...
class FavoritesService {
  Future<void> addToFavorites(String userId, Recipe recipe) async {
    try {
      var res = await supabase
          ...
          .insert({'user_uuid': userId, 'recipe_id': recipe.recipeId}).select(
              'recipe_id');
      print('[Favorites Service] Recipe Favorited: $res');
    } catch (e) {
      print('[Favorites Service] Error favoriting recipe: $e');
      rethrow;
    }
  }

  Future<void> removeFromFavorites(String userId, Recipe recipe) async {
    try {
      var res = await supabase
          .from('user_favourites')
          .delete()
      ...
      print('[Favorites Service] Recipe Unfavorited');
    } catch (e) {
      print('[Favorites Service] Error unfavoriting recipe: $e');
    }
  }
  ...
  Future<dynamic> fetchFavorites(String userId) async {
    try {
      ...
      print('[Favorites Service] Error fetching favorites: $e');
    }
  }
}
